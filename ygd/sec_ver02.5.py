...
#f=open("sec_input02.4.txt", 'r',encoding='utf8') #d드라이브 python폴더의 sec_input.txt을 읽어드림
f=open(filepath, 'r',encoding='utf8')
line=f.readlines()
f.close()

sentence=[]                        #리스트를 초기화 하여 sentence에 지정
for iresult in line[1:]:           #모든줄의 리스트 요소[0번째,1번째,2번째....]중 1번째부터 세어서 개체수 만큼 반복 (즉 0이면 공백줄이 없고, 1이면 첫번째줄까지 공백을 2이면 두번째줄까지 공백 의미)
  #isplt=iresult.split()            #isplt의 요소를 순차적으로 문자열에서 공백을 기준으로 리스트를 분할
  isplt=iresult.split(',')
  sentence.append(isplt)           #리스트 속의 리스트 작성[[],[],[]...](append함수 이용)
  #isplt=sentence                   #isplt에 하나의 리스트로 변수지정
...
for isen in sentence:
  print(isen)
  secname=isen[0]
  fck=int(isen[1])           #fck설정
  fy=int(isen[2])            #fy설정
  Øc=float(isen[3])            #Øc설정
  Øs=float(isen[4])            #Øs설정
  Mu=float(isen[5])            #Mu설정
  Vu=float(isen[6])            #Vu설정  
  Nu=float(isen[7])            #Nu설정
  Ms=float(isen[8])            #Ms설정
  H=float(isen[9])             #단면두께 설정
  B=float(isen[10])             #단면 폭 설정
  Dc=float(isen[11])           #피복두께 설정
  AsDia=int(isen[12])          #철근직경 설정
  AsNum=int(isen[13])          #철근개수 설정
  δ=float(isen[14])            #재분배 모멘트율 설정
  AvDia=int(isen[15])          #전단철근 직경
  AvLeg=float(isen[16])        #전단철근 다리개수
  AvSpace=int(isen[17])        #전단철근 배치간격
...
#------------------------------
  ...
  BB = -fyd * D
  C = Mu * 1000000
  asreq = (-BB - (BB**2 - 4*A*C)**(1/2)) / (2 * A)
  

  Asreq=ρreq*(B*D)
  print("asreq1:", asreq, "Asreq:",Asreq)
  if Asreq < Asuse:
    ch= "< 사용철근량...O.K"
  else:
    ch= "> 사용철근량...N.G"    
...
#--------------------------------------
  ...
  crackcheck=f"""   ▷ 사용성 검토(균열검토)
      Ms = {Ms:2.3f}kN.m  (사용하중조합-Ⅰ)
    - 균열발생 여부 검토 (비균열 단면으로 가정)"""
  ...
  crackcheck+=f"""      n  = Es/Ec = 200,000 / (0.077 mc^1.5 * ³√(Fck + Δf)) = {nr:2d}
      Xo = (B·H²/2 + (n-1)·As·D) / (B·H + (n-1)·As) = {Xo:2.3f}mm
      Io = B·H³/12 + B·H·(H/2-Xo)²+ (n-1)·As·(D-Xo)²= {Io:2.3f}mm⁴"""
  ...
  if fct <= fctk :
    ...
    crackcheck+=f"""      fct = Ms / Io×(H-Xo) = {fct:2.3f}MPa {cr1:2s} fctk = {fctk:2.3f}MPa {cr2:2s} \n
    - 철근응력제한 검토\n
      fs = n·Ms / Io×(D-Xo) =  {fs:2.3f}MPa {cr3:2s} 0.8fy = {0.8*fy:2.3f}MPa {cr4:2s}\n\n"""
  else:
    cr2="∴ 균열 단면 검토"
    ...
    - 철근응력제한 검토\n
      ρ = As/(B·D) =  {p:2.5f}\n
      k  = -nρ+ √((nρ)²+ 2nρ) = {k:2.3f}  (j= 1-k/3= {1-k/3:2.3f})\n
      x  = k·d =  {x:2.5f}mm\n
      fc = 2·Ms / (B·x·(D - x/3)) =  {fc:2.5f}MPa\n
      fs = Ms / (As ·(D - x/3))   =  {fs:2.5f}MPa {cr3:2s} 0.8fy = {0.8*fy:2.3f}MPa {cr4:2s}\n\n"""
    
    crackcheck+=f"""    - 간접균열제어 및 최소철근량\n
      철 근 직 경 : {AsDia:2d}mm,    철 근 간 격 : {(B-2*Dc)/AsNum:2.3f}mm\n"""
    ...
    if fsa >= fs:
        ...
        cr6="... ∴N.G"
    crackcheck+=f"""      허 용 응 력 : fsa = Max(160,360) = {fsa:2d}MPa {cr5:2s} fs = {fs:2.3f}MPa {cr6:2s}\n"""
    ...
    if Asmin <= Asuse:
        ...
        cr8="... ∴N.G"
    crackcheck+=f"""      최소 철근량 : As.min = kc·k·Act·fct / fsa = {Asmin:2.3f} ㎟ {cr7:2s} As={Asuse:2.3f}㎟ {cr8:2s}\n
            여기서... Act = B·(H-Xo) = {Act:2.3f} ㎟\n
                      kc  = 0.4×[1 - fn/(k₁(h/h*)×fct)] = {kc:2.3f}\n    
                      k   =  {k:2.3f}\n
                      fct = fctm = {fct:2.3f}MPa\n  
                      fsa = {fsa:2.3f}MPa\n"""
...
◈ 직사각형 보\n\n
  ...
  outputstr+=shearchk
  outputstr+=crackcheck

wfilepath = os.path.splitext(filepath)[0]+'.out'
#f=open("section_check.txt", 'w',encoding='utf8')
f=open(wfilepath, 'w',encoding='utf8')
f.write(outputstr)
f.close()

print('새로 생성된 "section_check.out"를 확인하세요!!')